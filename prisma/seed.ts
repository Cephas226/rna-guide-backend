// ============================================================
// RNA Guide - Seed de données réalistes (Burkina Faso)
// Exécuter: npm run seed
// ============================================================

import { PrismaClient, Role, SyncStatus, ProductType, HealthState, Season } from '@prisma/client';
import * as bcrypt from 'bcryptjs';

const prisma = new PrismaClient();

// ─────────────────────────────────────────
// DONNÉES GÉOGRAPHIQUES RÉELLES BF
// ─────────────────────────────────────────
...
function generateGPSNearPoint(baseLat: number, baseLng: number, radiusKm = 0.5) {
...
function generateParcelGPS(centerLat: number, centerLng: number, areaHa: number) {
  // Approximation: 1 ha ≈ 100m x 100m → 0.001° x 0.001°
  ...
  return [
    ...
    { lat: centerLat + halfSide, lng: centerLng + halfSide },
    { lat: centerLat - halfSide, lng: centerLng + halfSide },
    { lat: centerLat - halfSide, lng: centerLng - halfSide }, // fermer le polygone
  ];
}
...
async function main() {
  ...
  // Superviseurs (1 par région)
  const superviseurs: any[] = [];
  ...
  // Producteurs (50 total)
  const producteurs: any[] = [];
  for (let i = 0; i < 50; i++) {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const syncStatuses = [SyncStatus.SYNCED, SyncStatus.SYNCED, SyncStatus.SYNCED, SyncStatus.PENDING];
    ...
    // Photo points pour chaque parcelle
  ...
  // ── 5. INVENTAIRES (1-2 par parcelle, années 2022-2025) ──
  ...
  for (const { parcel } of parcelles) {
    ...
    for (const year of selectedYears) {
      const season = random([Season.HIVERNAGE, Season.SAISON_SECHE]);
      ...
      const speciesData = selectedSpeciesIds.map(speciesId => {
        ...
        return {
          ...
          totalPieds: total,
          selectedPieds: selected,
          healthState: random([HealthState.BON, HealthState.BON, HealthState.MOYEN, HealthState.MAUVAIS]) as HealthState,
  ...
  for (const { parcel, photoPoints, gps } of parcelles.slice(0, 60)) {
    ...
    for (let i = 0; i < nbPhotos; i++) {
      const pp: any = photoPoints.length > 0 ? random(photoPoints) : null;
  ...
  const PRODUITS_CONFIG = [
    { type: ProductType.FRUITS, units: ['kg', 'sac', 'botte'], priceRange: [150, 800] },
    { type: ProductType.FEUILLES, units: ['kg', 'fagot', 'botte'], priceRange: [50, 300] },
    { type: ProductType.BOIS, units: ['fagot', 'stère', 'kg'], priceRange: [500, 2500] },
    { type: ProductType.GOUSSES, units: ['kg', 'sac'], priceRange: [200, 600] },
    { type: ProductType.COMBUSTIBLE, units: ['fagot', 'kg'], priceRange: [300, 1000] },
    { type: ProductType.ECORCES, units: ['kg', 'botte'], priceRange: [400, 1500] },
  ];
